          ...
          <div key={c.title} className={cn(card, cardInner, 'flex flex-col justify-between gap-4')}>
            <div>
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <h4 className="text-sm font-bold text-[hsl(var(--admin-text))]">{c.title}</h4>
              </div>
              <p className="text-xs text-[hsl(var(--admin-text-muted))] leading-relaxed">{c.description}</p>
            </div>

            {c.stats ? (
              <div className="rounded-lg border border-[hsl(var(--admin-card-border))] bg-[hsl(var(--admin-bg)/0.5)] p-3 space-y-1.5">
                {c.stats.map(s => (
                  <div key={s.label} className="flex justify-between text-xs">
                    <span className="text-[hsl(var(--admin-text-muted))]">{s.label}</span>
                    <span className="font-bold text-[hsl(var(--admin-text))]">{s.value}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="flex justify-center py-3"><Loader2 className="h-4 w-4 animate-spin text-[hsl(var(--admin-text-muted))]" /></div>
            )}

            <Button asChild size="sm" variant="outline" className="w-full justify-between h-9 border-[hsl(var(--admin-card-border))] text-[hsl(var(--admin-text-muted))] hover:text-[hsl(var(--admin-text))] hover:bg-[hsl(var(--admin-bg))]">
